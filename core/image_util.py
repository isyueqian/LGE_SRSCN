...
class BaseDataProvider(object):
    ...
    def _load_data_and_label(self):
        data, label, affine = self._next_data()
        ...
        if self.inference_phase:
            return train_data.reshape(1, nx, ny, nz, self.channels), labels.reshape(1, nx, ny, nz, self.n_class), affine
        else:
            return train_data.reshape(1, nx, ny, self.channels), labels.reshape(1, nx, ny, self.n_class), affine
    ...
    def __call__(self, n):
        if self.crop_patch:
            train_data, labels, affine = self._load_data_and_label()
